...
build: $(BIN_REST) $(BIN_GRPC) $(BIN_MONITOR)
...
_bench-rest:
	...
	REST_PID=$$!; \
	trap 'kill $$REST_PID 2>/dev/null || true; kill $$MON_PID 2>/dev/null || true' EXIT INT TERM; \
	echo "[bench] REST démarré (PID=$$REST_PID), attente 1s..."; \
	sleep 1; \
